        ...
        for item in nested[:8]:
            if isinstance(item, dict):
                ...
                if isinstance(sc, (int, float)):
                    ...
                    # si le score semble être sur 10 (<=10), le convertir
                    pct = float(sc)
                    if 0 < pct <= 10:
                        pct *= 10
...
def build_search_process_lines(
    ...
    if semantic_meta.get("no_exact_match"):
        ...
        lines.append("Re-classement par adéquation poste (30% skills, 35% sémantique, …)")
...
def parse_recruiter_filters(question: str) -> Dict[str, Any]:
    """Filtres simples AND / NOT / expérience min / master / score."""
    q = question.lower()
    filt: Dict[str, Any] = {
        "min_years": None,
        "max_years": None,
    ...
    # "N+ years" pattern
    m3 = re.search(r"(\d+)\s*\+\s*(?:years?|ans?)" , q)
    if m3 and filt["min_years"] is None:
        filt["min_years"] = int(m3.group(1))
    # "more than N years" pattern
    m4 = re.search(r"more than\s+(\d+)\s*(?:years?|ans?)" , q)
    if m4 and filt["min_years"] is None:
        filt["min_years"] = int(m4.group(1))
    # Maximum experience patterns (e.g. "less than 2 years", "junior")
    m_max = re.search(r"(?:less than|moins de)\s+(\d+)\s*(?:an|ans|year)", q)
    if m_max:
        filt["max_years"] = int(m_max.group(1))

    # Score patterns
    m_score_min = re.search(r"(?:score|>|supérieur à|au moins|minimum|plus de)\s*(\d+(?:\.\d+)?)", q)
    if "score" in q and m_score_min:
        try:
            val = float(m_score_min.group(1))
            if val > 10 and val <= 100:  # e.g., 70
                filt["min_score"] = val
            elif val <= 10: # e.g., 7/10 -> wait, global score is up to 100
                filt["min_score"] = val * 10 
        except ValueError:
            pass

    if re.search(r"master|mastère|m2\b|bac\+5", q):
    ...
    # "exclude" / "without" patterns
    if re.search(r"\bexclude\b|\bwithout\b", q):
        neg_part = re.split(r"\bexclude\b|\bwithout\b", q, maxsplit=1)
        if len(neg_part) == 2:
            extra = extract_query_skills(neg_part[1])
            filt["must_not"].extend(s for s in extra if s not in filt["must_not"])
...
def build_es_filter_from_constraints(filt: Dict[str, Any]) -> List[Dict]:
    """Convertit les filtres en clauses Elasticsearch `filter`."""
    must_clauses = []
    
    # Expérience
    if filt.get("min_years") is not None or filt.get("max_years") is not None:
        rng = {}
        if filt.get("min_years") is not None:
            rng["gte"] = filt["min_years"]
        if filt.get("max_years") is not None:
            rng["lte"] = filt["max_years"]
        must_clauses.append({"range": {"annees_experience": rng}})
        
    # Score
    if filt.get("min_score") is not None or filt.get("max_score") is not None:
        rng = {}
        if filt.get("min_score") is not None:
            rng["gte"] = filt["min_score"]
        if filt.get("max_score") is not None:
            rng["lte"] = filt["max_score"]
        must_clauses.append({"range": {"score_qualite_globale": rng}})
        
    # Must skills
    for skill in filt.get("must_skills", []):
        must_clauses.append({
            "multi_match": {
                "query": skill,
                "fields": ["technologies", "langages", "frameworks", "text"],
                "type": "phrase"
            }
        })

    # Must NOT skills
    # Elasticsearch `bool -> must_not` is better implemented by the caller wrapping this list
    # But since we just return a filter list (which is implicitly ANDed by `filter: []`),
    # To do a MUST NOT, we must wrap it in a bool inside the filter list.
    for skill in filt.get("must_not", []):
        must_clauses.append({
            "bool": {
                "must_not": {
                    "multi_match": {
...
def apply_recruiter_filters(docs: List[Dict], filt: Dict[str, Any]) -> List[Dict]:
    ...
    for d in docs:
        ...
        if filt.get("max_years") is not None:
            exp = d.get("annees_experience")
            if isinstance(exp, (int, float)) and exp >= filt["max_years"]:
                continue
