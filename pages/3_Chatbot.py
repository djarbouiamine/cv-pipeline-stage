...
from chatbot.recruiter_helpers import (
    ...
    build_es_filter_from_constraints,
...
def retrieve_top_k(es, query_vec: np.ndarray, k: int = 8, es_filter: Optional[List[Dict]] = None) -> List[Dict]:
    knn_clause = {
        "field": "embedding_cv",
...
def retrieve_all_sorted(es, sort_field: str = "score_qualite_globale",
                        order: str = "desc",
                        es_filter: Optional[List[Dict]] = None) -> List[Dict]:
    total = es.count(index="cvs")["count"]
    query = {"match_all": {}}
    if es_filter:
        query = {"bool": {"must": query, "filter": es_filter}}
    res = es.search(
        index="cvs",
        size=total,
        query=query,
...
def retrieve_top_n(es, sort_field: str = "score_qualite_globale",
    ...
    [FIX-CAT] Quand une catégorie est demandée mais qu'aucun candidat
    ne l'a dans la base, retourne une liste contenant un marqueur
    spécial {'_category_not_found': True, '_requested_category': cat}
    au lieu de retourner une liste vide (qui déclenchait auparavant
    un fallback sémantique trompeur sur TOUS les candidats).
    """
    es_cat = None
    if category:
    ...
    # Incorporate es_filter using bool
    if es_filter:
        query = {
            "bool": {
                "must": query,
    ...
    if category and not es_cat:
        ...
        if kw:
            # Note: topic fallback doesn't currently support es_filter, could be enhanced
            results = retrieve_by_topic_keywords(es, kw)[:n]
            if results:
                return results
        # Aucun candidat trouvé pour cette catégorie
        return [{"_category_not_found": True, "_requested_category": category}]
    ...
    docs = [dict(h["_source"]) for h in res["hits"]["hits"]]
    if not docs and es_cat:
        # La catégorie existe dans l'index ES mais aucun CV ne correspond
        return [{"_category_not_found": True, "_requested_category": category or es_cat}]
    return _attach_none_score(docs)


def retrieve_by_category(es, category: str, es_filter: Optional[List[Dict]] = None) -> List[Dict]:
    ...
    if not es_cat:
        ...
        if keywords:
            return retrieve_by_topic_keywords(es, keywords, es_filter=es_filter)
        return []
...
def retrieve_by_topic_keywords(es, keywords: List[str], es_filter: Optional[List[Dict]] = None) -> List[Dict]:
    if not keywords:
        return []
    total = es.count(index="cvs")["count"]
...
def enrich_semantic_retrieval(
    ...
    # [FIX-10] Zero-skill check: if the question mentions specific skills
    # (recognized in KNOWN_SKILLS) but NO CV in the result set has ANY of them,
    # this is likely a skill that doesn't exist in the database.
    # Return empty results with a clear "no match" signal instead of
    # returning all semantic results that are only vaguely related.
    query_skills_list = meta["query_skills"]
    if query_skills_list and docs:
        any_match = False
        for d in docs:
            if not missing_skills(d, query_skills_list):
                any_match = True
                break
        if not any_match:
            # Check if even partial match exists (at least 1 skill found in any doc)
            partial_any = any(
                not missing_skills(d, [s]) for d in docs for s in query_skills_list
            )
            if not partial_any:
                # No CV has even ONE of the requested skills
                meta["no_exact_match"] = True
                meta["mandatory_label"] = "compétences : " + ", ".join(query_skills_list)
                for d in docs:
                    meta["rejected"].append({
                        "nom": d.get("nom", "?"),
                        "reason": f"Aucune des compétences demandées ({', '.join(query_skills_list)}) trouvée dans ce CV",
                    })
                docs = []

    # [FIX-12] Detect impossible/paradoxical questions
    q_lower = question.lower()
    impossible_patterns = [
        (r"skills that are not mentioned", "compétences non mentionnées explicitement"),
        (r"skills.*not.*explicitly.*(?:in|mentioned|listed)", "critère impossible à vérifier"),
        (r"information.*not.*(?:in|available|present)", "information potentiellement absente des CVs"),
    ]
    for pat, reason in impossible_patterns:
        if re.search(pat, q_lower):
            meta["no_exact_match"] = True
            if not meta["mandatory_label"]:
                meta["mandatory_label"] = reason
            break
...
def route_question(question: str, es) -> Tuple[Optional[List[Dict]], Optional[Dict], str, str]:
    ...
    filt = parse_recruiter_filters(question)
    es_filter = build_es_filter_from_constraints(filt)
    ...
    try:
        ...
        if mode_llm == "comparison":
            ...
            n = top_n_llm
            docs = retrieve_top_n(es, sort_field=sort_field_llm, order="desc", n=n, category=category_llm, es_filter=es_filter)
            label = f"🏆 Classement top {n} par {field_label_llm}"
            if category_llm:
                label += f" en {category_llm}"
            # [FIX-CAT] Propager la catégorie demandée dans la description
            # pour que le handler principal puisse détecter le cas "aucun
            # candidat dans cette catégorie" et afficher un message clair.
            desc_with_cat = f"{label}{conf_badge}"
            if category_llm:
                desc_with_cat = f"[CAT:{category_llm}] {desc_with_cat}"
            return docs, None, "classement", desc_with_cat
            ...
            if category_llm or topic_label:
                ...
                docs = retrieve_by_category(es, label or category_llm, es_filter=es_filter)
                # [FIX-CAT] Propager la catégorie demandée
                return docs, None, "filter", f"[CAT:{label or category_llm}] 🏷️ CVs filtrés : {label}{conf_badge}"
            docs = retrieve_all_sorted(es, sort_field=sort_field_llm, es_filter=es_filter)
            ...
            docs = retrieve_top_k(es, q_vec, k=k, es_filter=es_filter)
    ...
    if contains_keyword(q, MOTS_CLES_CLASSEMENT):
        ...
        docs = retrieve_top_n(es, sort_field=sort_field, order="desc", n=n, category=category, es_filter=es_filter)
        label = f"🏆 Classement des top {n} CVs par {field_label}"
        if category:
            label += f" en {category}"
        # [FIX-CAT] Propager la catégorie pour détection du cas "aucun candidat"
        desc = f"[CAT:{category}] {label}" if category else label
        return docs, None, "classement", desc
    ...
    if contains_keyword(q, MOTS_CLES_FILTRAGE):
        category = detect_category_in_question(question)
        if category:
            docs = retrieve_by_category(es, category, es_filter=es_filter)
            # [FIX-CAT] Propager la catégorie pour détection du cas "aucun candidat"
            return docs, None, "filter", f"[CAT:{category}] 🏷️ CVs filtrés par catégorie : {category}"
        sort_field, _ = detect_sort_field(question)
        docs = retrieve_all_sorted(es, sort_field=sort_field, es_filter=es_filter)
    ...
    if isinstance(desc_projets, list) and desc_projets:
        ...
        for item in desc_projets[:4]:
            if isinstance(item, dict):
                ...
                desc_parts.append(f"{nom_p}: {desc_p}")
            else:
...
inject_theme()
hero("💬", "Assistant RAG — Conversation &amp; Recherche CV", "Mode automatique : questions générales OU recherche dans vos CVs indexés", badge="🤖 Dual-mode")
...
if question and question.strip():
    ...
    if user_intent["intent"] == "general":
        ...
        # [FIX-CAT] Extraire la catégorie demandée depuis la description (marqueur [CAT:...])
        _requested_cat_match = re.search(r"\[CAT:([^\]]+)\]", mode_description)
        _requested_cat = _requested_cat_match.group(1).strip() if _requested_cat_match else None
        if _requested_cat_match:
            mode_description = mode_description.replace(_requested_cat_match.group(0), "").strip()

        # [FIX-CAT] Détecter le marqueur _category_not_found retourné par retrieve_top_n
        _cat_not_found = (
            docs
            and len(docs) == 1
            and docs[0].get("_category_not_found")
        )

        if _cat_not_found:
            # Aucun candidat dans la catégorie demandée → message explicite,
            # PAS de fallback sémantique qui montrerait des candidats d'autres catégories
            cat_label = docs[0].get("_requested_category") or _requested_cat or "cette catégorie"
            known_cats = get_known_categories(es)
            cats_str = ", ".join(known_cats) if known_cats else "(aucune catégorie connue)"
            answer = (
                f"⚠️ **Aucun candidat trouvé dans la catégorie « {cat_label} ».**\n\n"
                f"La base de données ne contient pas de CV classé dans cette catégorie.\n"
                f"Catégories disponibles : {cats_str}\n\n"
                f"Essayez de reformuler avec l'une de ces catégories, ou posez une question "
                f"sur les compétences directement (ex : \"Top 5 en Intelligence Artificielle\")."
            )
            mode = "classement"
            docs = []
            sources_df = pd.DataFrame()
            semantic_meta = {}
            count_line = format_cv_count_line(get_total_cv_count(es), 0, 0)
            trace_badge = (
                f"🛡️ **Anti-hallucination** : {count_line}. "
                f"CV Quality = stored in database · Job Fit = computed for this query."
            )
        elif (not docs) and mode != "stats":
            # [FIX-CAT] Si une catégorie était demandée et qu'on n'a rien trouvé,
            # NE PAS faire un fallback sémantique sur tous les candidats.
            if _requested_cat:
                known_cats = get_known_categories(es)
                cats_str = ", ".join(known_cats) if known_cats else "(aucune catégorie connue)"
                answer = (
                    f"⚠️ **Aucun candidat trouvé dans la catégorie « {_requested_cat} ».**\n\n"
                    f"La base de données ne contient pas de CV classé dans cette catégorie.\n"
                    f"Catégories disponibles : {cats_str}\n\n"
                    f"Essayez de reformuler avec l'une de ces catégories."
                )
                mode = mode
                docs = []
                sources_df = pd.DataFrame()
        ...
        # _answer_already_set = True when the category-not-found handler already composed the answer
        _answer_already_set = _cat_not_found or (bool(_requested_cat) and not docs)

        if (not docs) and mode != "stats" and not _answer_already_set:
            answer = (
